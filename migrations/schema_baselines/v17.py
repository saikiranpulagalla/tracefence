"""Frozen v17 schema contract used exclusively by migration 001.

This metadata mirrors the exact v17 repository snapshot. Future application-model
changes must never modify it; later schema evolution belongs only in later Alembic
migrations.
"""
...
class CredentialRecoveryEnvelope(V17Base):
...
class CorrectionProposal(V17Base):
...
class CommandAcknowledgement(V17Base):
...
class ActionCommandMatch(V17Base):
...
class InvariantViolation(V17Base):
...
# Frozen exports for migration 001 only; never import current application metadata.
V17_METADATA = V17Base.metadata
V17_SCHEMA_METADATA = SchemaMetadata.__table__
