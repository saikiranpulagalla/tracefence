"""Frozen historical schema contract.

This module is owned by the migration identified in its filename. Future
application-model changes must never modify it; later schema evolution belongs
in later Alembic migrations. It intentionally imports no current application
ORM metadata, mapped classes, enums, or DDL installers.
"""
...
class V22_BASE(DeclarativeBase):
    pass


class SchemaMetadata(V22_BASE):
...
class Node(V22_BASE):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(["run_id", "supersedes_node_id"], ["nodes.run_id", "nodes.id"]),
        ForeignKeyConstraint(
        ...
        CheckConstraint(
            "(generation = 0 AND parent_id IS NULL) OR (generation > 0 AND parent_id IS NOT NULL)",
...
class ControlScope(V22_BASE):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("run_id", "id", "owner_node_id", name="uq_scope_run_owner"),
...
class CredentialRecoveryEnvelope(V22_BASE):
...
class CorrectionProposal(V22_BASE):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(["run_id", "reviewed_by_node_id"], ["nodes.run_id", "nodes.id"]),
        ForeignKeyConstraint(
        ...
        CheckConstraint("proposal_type IN ('CANCEL','CORRECT')", name="ck_proposal_type"),
    ...
    authorized_command_digest: Mapped[str | None] = mapped_column(String(64), nullable=True)
    resulting_command_id: Mapped[str | None] = mapped_column(String(36), nullable=True)
    reviewed_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=utcnow)


class ControlCommand(V22_BASE):
    __tablename__ = "control_commands"
    __table_args__ = (
        UniqueConstraint(
            "run_id", "issuer_fingerprint", "idempotency_key", name="uq_command_idempotency"
        ),
        ForeignKeyConstraint(["run_id", "issuer_node_id"], ["nodes.run_id", "nodes.id"]),
        ForeignKeyConstraint(["run_id", "target_node_id"], ["nodes.run_id", "nodes.id"]),
        ForeignKeyConstraint(
            ["run_id", "target_scope_id"], ["control_scopes.run_id", "control_scopes.id"]
        ),
        ForeignKeyConstraint(["run_id", "replacement_parent_id"], ["nodes.run_id", "nodes.id"]),
        ForeignKeyConstraint(["run_id", "replacement_node_id"], ["nodes.run_id", "nodes.id"]),
        ForeignKeyConstraint(
        ...
        UniqueConstraint("run_id", "id", "to_version", name="uq_command_exact_version"),
        CheckConstraint("issuer_type IN ('HUMAN','AGENT')", name="ck_command_issuer_type"),
        CheckConstraint(
...
class CommandAcknowledgement(V22_BASE):
    __tablename__ = "command_acknowledgements"
    __table_args__ = (
        UniqueConstraint("run_id", "command_id", "node_id", "ack_type", name="uq_command_ack"),
        ForeignKeyConstraint(
            ["run_id", "command_id"],
            ["control_commands.run_id", "control_commands.id"],
            ondelete="CASCADE",
...
class ActionCommandMatch(V22_BASE):
...
class InvariantViolation(V22_BASE):
...
class TelemetryOutbox(V22_BASE):
...
class RuntimeStopIntent(V22_BASE):
...
class RuntimeStopTarget(V22_BASE):
...
V22_METADATA = V22_BASE.metadata
V22_RUNTIME_STOP_INTENT = RuntimeStopIntent.__table__
V22_RUNTIME_STOP_TARGET = RuntimeStopTarget.__table__
