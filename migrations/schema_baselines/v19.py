...
class V19_BASE(DeclarativeBase):
    pass


class SchemaMetadata(V19_BASE):
...
class CredentialRecoveryEnvelope(V19_BASE):
...
class CorrectionProposal(V19_BASE):
...
class CommandAcknowledgement(V19_BASE):
...
class ActionCommandMatch(V19_BASE):
...
class InvariantViolation(V19_BASE):
...
class TelemetryOutbox(V19_BASE):
...
V19_METADATA = V19_BASE.metadata
V19_WORKER_INSTANCE = WorkerInstance.__table__


V19_WORKER_INSTANCE_TRIGGER_DDL = (
    """
    CREATE TRIGGER IF NOT EXISTS trg_worker_instances_id_immutable
    BEFORE UPDATE OF id ON worker_instances
    WHEN NEW.id != OLD.id
