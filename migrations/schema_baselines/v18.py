...
class V18_BASE(DeclarativeBase):
    pass


class SchemaMetadata(V18_BASE):
...
class CredentialRecoveryEnvelope(V18_BASE):
...
class CorrectionProposal(V18_BASE):
...
class CommandAcknowledgement(V18_BASE):
...
class ActionCommandMatch(V18_BASE):
...
class InvariantViolation(V18_BASE):
...
class TelemetryOutbox(V18_BASE):
...
V18_METADATA = V18_BASE.metadata
V18_RUNTIME_EVENT = RuntimeEvent.__table__


V18_RUNTIME_EVENT_TRIGGER_DDL = (
    """
    CREATE TRIGGER trg_runtime_events_no_update
    ...
    """,
    """
    CREATE TRIGGER trg_runtime_events_no_delete
