...
class V23_BASE(DeclarativeBase):
    pass


class SchemaMetadata(V23_BASE):
...
class CredentialRecoveryEnvelope(V23_BASE):
...
class CorrectionProposal(V23_BASE):
...
class CommandAcknowledgement(V23_BASE):
...
class ActionCommandMatch(V23_BASE):
...
class InvariantViolation(V23_BASE):
...
class TelemetryOutbox(V23_BASE):
...
class RuntimeStopIntent(V23_BASE):
...
class RuntimeStopTarget(V23_BASE):
...
V23_METADATA = V23_BASE.metadata
V23_WORKER_STOP_TASK = WorkerStopTask.__table__
